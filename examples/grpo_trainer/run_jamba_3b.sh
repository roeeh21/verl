...
if [ ! -d "$MODEL_PATH" ]; then
    mkdir -p "$MODEL_PATH"
    gcloud storage cp -r "gs://ai21-algo-studio-checkpoints/conversions/hf/model_name=sft-jamba-3b-nv-cp175-j17-a2-hotels-lr1e-05-seq32768-bs64/checkpoint=5000/*" "$MODEL_PATH"
fi

GPUS=4
# sync is regular, async will activate agent loop
ROLLOUT_MODE=async
# Enforce eager should be enabled if and only if rollout mode is async
ENFORCE_EAGER=True
