...
############################################################################################################
##### TAMERG: this file was deprecated in v0.5.0. We are keeping it here for workflow compatibility.
############################################################################################################
...
parser.add_argument(
    ...
    required=True,
    help=(
        "The path for your saved model. For megatron, point to the base dir of model, rng,"
        " optimizer checkpoints, commonly be `config.default_local_dir/global_step_{global_step}`."
    ),
)
parser.add_argument(
    "--target_dir", required=False, type=str, help="The path for the target model"
)  # TODO: upstream has default /tmp target dir, therefore as long as we don't specify the
# target dir the model is copied to ephemeral storage instead of dev/shm/huggingface as intended
parser.add_argument("--hf_upload_path", default=False, type=str, help="The path of the huggingface repo to upload")
parser.add_argument("--test", action="store_true", help="test correctness of hf_model")
parser.add_argument(
    "--delete-original",
    action="store_true",
    help="Delete original checkpoint shards",
)
parser.add_argument(
    "--test_hf_dir",
    type=str,
...
os.makedirs(args.target_dir, exist_ok=True) if args.target_dir else None
if args.test:
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    if args.delete_original:
        os.remove(os.path.join(local_dir, f"model_world_size_{world_size}_rank_{rank}.pt"))
