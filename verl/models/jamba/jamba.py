    ...
    def __init__(self, config: JambaConfig, layer_idx: Optional[int] = None):
        ...
        if layer_idx is None:
            logger.warning_once(
                ...
                "without passing a `layer_idx` is not recommended and will lead to errors during the forward call "
...
class JambaFlashAttention2(JambaAttention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Beware that with flash_attn<2.1, using q_seqlen != k_seqlen (except for the
        # case q_seqlen == 1) produces a wrong mask (top-left).
        ...
        **kwargs,
        ...
        if input_dtype == torch.float32:
            ...
            logger.warning_once(
                ...
                "related to the fact you have upcasted embedding or layer norm layers in float32. "
...
class JambaSdpaAttention(JambaAttention):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "JambaModel is using JambaSdpaAttention, but "
                "`torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. "
                "Falling back to the manual attention implementation, but specifying the manual implementation will "
                "be required from Transformers version v5.0.0 onwards. This warning can be removed using the "
        ...
        # inputs with custom attn_mask, Reference:
        ...
        # instead of an inline conditional assignment in SDPA to support both torch.compile's dynamic
        # shapes and full graph options. An inline conditional prevents dynamic shapes from compiling.
...
class JambaMambaMixer(nn.Module):
    ...
    def __init__(self, config: JambaConfig, layer_idx):
        ...
        if not is_fast_path_available:
            logger.warning_once(
                ...
                "causal_conv1d_update, mamba_inner_fn) is None. To install, follow "
                "https://github.com/state-spaces/mamba/#installation and "
                "https://github.com/Dao-AILab/causal-conv1d. To use the naive implementation, set "
                "`use_mamba_kernels=False` in the model config."
            )
    ...
    def slow_forward(
        ...
        if use_cache and cache_params.ssm_states[self.layer_idx].shape[0] == batch_size:
            if self.training:
                # In training mode, we don't want to perform in-place operations on ssm_state
                # so we can compute the backwards pass
    ...
    def forward(
        ...
        if self.use_fast_kernels:
            if not is_fast_path_available or "cuda" not in self.x_proj.weight.device.type:
                raise ValueError(
                    "Fast Mamba kernels are not available. Make sure they are installed and that the mamba "
                    "module is on a CUDA device"
                )
...
class JambaSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: JambaConfig):
        ...
        self.router = JambaRouter(self.hidden_dim, self.num_experts, bias=False)
        self.experts = nn.ModuleList([JambaMLPMoE(config) for _ in range(self.num_experts)])
...
class JambaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear | nn.Conv1d):  # noqa: UP038
...
JAMBA_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (
            `HybridMambaAttentionDynamicCache`, *optional*, returned when `use_cache=True` is passed or when
...
class JambaModel(JambaPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache and past_key_values is None:
            logger.warning_once(
                "Jamba requires an initialized `HybridMambaAttentionDynamicCache` to return a cache. "
        ...
        **loss_kwargs,
        ...
        r"""
            ...
            logits_to_keep (`int` or `torch.Tensor`, *optional*):
                ...
                `input_ids` (special case). Only last token logits are needed for generation, and calculating them
                only for that token can save memory, which becomes pretty significant for long sequences or large
                vocabulary size.
...
__all__ = ["JambaForCausalLM", "JambaModel", "JambaPreTrainedModel"]
