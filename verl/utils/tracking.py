...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tensorboard" in default_backend:
            self.logger["tensorboard"] = _TBLoggingAdapter(
                config["trainer"]["default_local_dir"],
                config["trainer"]["remote_save_path"],
                config["trainer"]["log_freq"],
            )

            # TODO: TAMERG: why we don't use the native TB adapter?
...
class _TBLoggingAdapter:
    def __init__(self, local_path: str, remote_path: str, log_freq: int):
    ...
    def _ensure_summary_writers(self):
        if self.local_sw is not None:
            return

        from tensorboardX import SummaryWriter

        self.local_sw = SummaryWriter(self.local_path)
        if self.remote_path:
            self.remote_sw = SummaryWriter(self.remote_path)

    def get_agg_metrics(self, data, step):
        for k, v in data.items():
            if k not in self.metrics_arr:
                self.metrics_arr[k] = []
            self.metrics_arr[k].append(v)

        if step % self.log_freq == 0:
            agg_metrics = {
                k: np.mean(self.metrics_arr[k])
                for k in self.metrics_arr
                if np.issubdtype(type(self.metrics_arr[k][0]), np.number)
            }
            self.metrics_arr = {}
            return agg_metrics
        else:
    ...
    def log(self, data, step):
        agg_metrics = self.get_agg_metrics(data, step)
        if agg_metrics is None:
            return

        self._ensure_summary_writers()

        for k, v in agg_metrics.items():
            if v == v:  # check if nan
                self.local_sw.add_scalar(k, v, step)
                if self.remote_sw is not None:
                    self.remote_sw.add_scalar(k, v, step)

    def finish(self):
        if self.local_sw is None:
            return

        self.local_sw.close()
        if self.remote_sw is not None:
            self.remote_sw.close()
