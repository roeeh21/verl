...
DEFAULT_GPU_MEMORY_LOGGER_LEVEL = getattr(logging, os.environ.get("GPU_MEMORY_LOGGER_LEVEL", "DEBUG"))


def log_nvidia_smi_gpu_memory_usage(head: str, logger: logging.Logger = None, level=DEFAULT_GPU_MEMORY_LOGGER_LEVEL):
    """
    Useful for logging GPU memory stats from main ray training loop,
    which is not assigned to a device and thus can't use
    torch.cuda.memory_X functions.
    """
    divisor = 1024**3  # GB
    mem_nv = list(
        map(
            int,
            subprocess.check_output("nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits".split())
            .decode("utf-8")
            .strip()
            .split("\n"),
        )
    )
    values = [f"{mem * 1024**2 / divisor:.2f}" for mem in mem_nv]
    message = f"{head}, GPUs memory usage (GB): {' | '.join(values)}"
    if logger is None:
        print(message)
    else:
...
def _get_current_mem_info(unit: str = "GB", precision: int = 2) -> tuple[str]:
    ...
    values = [f"{mem * 1024**2 / divisor:.2f}" for mem in mem_nv]
    mem_nv = " | ".join(values)
    return mem_allocated, mem_reserved, mem_used, mem_total, mem_nv


def log_gpu_memory_usage(
    head: str, logger: logging.Logger = None, level=DEFAULT_GPU_MEMORY_LOGGER_LEVEL, rank: int = 0
):
    ...
    if (not dist.is_initialized()) or (rank is None) or (dist.get_rank() == rank):
        mem_allocated, mem_reserved, mem_used, mem_total, mem_nv = _get_current_mem_info()
        message = (
            f"{head}, memory allocated (GB): {mem_allocated}, memory reserved (GB): {mem_reserved}, "
            f"device memory used/total (GB): {mem_used}/{mem_total}, nvidia-smi (GB): {mem_nv}"
        )
...
class GPUMemoryLogger(DecoratorLoggerBase):
    ...
    def __init__(
        ...
        level=DEFAULT_GPU_MEMORY_LOGGER_LEVEL,
...
def _timer(name: str, timing_raw: dict[str, float]):
    ...
    with Timer(
        name=name,
        logger=print,
        initial_text="Timer started for: {name}",
        text="Timer ended for: {name}. took {seconds:0.2f} seconds ({minutes:0.2f} minutes)",
    ) as timer:
        yield
