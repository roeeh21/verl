...
from verl.utils.checkpoint.file_utils import sync_file_gs, sync_paths_gs_threaded
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False, **kwargs):
        """
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            if self.should_load_model:
                ...
                if remote_path is not None:
                    sync_file_gs(
                        os.path.join(remote_path, f"model_world_size_{self.world_size}_rank_{self.rank}.pt"),
                        remote_model_path,
                    )
                ...
                log_with_rank(
                    ...
                    + (f" and {remote_model_path}" if remote_path is not None else ""),
                    rank=self.rank,
                    logger=logger,
                )
            ...
            if self.should_load_optimizer:
                ...
                if remote_path is not None:
                    sync_file_gs(
                        os.path.join(remote_path, f"optim_world_size_{self.world_size}_rank_{self.rank}.pt"),
                        remote_optim_path,
                    )
                ...
                log_with_rank(
                    ...
                    + (f" and {remote_optim_path}" if remote_path is not None else ""),
                    rank=self.rank,
                    logger=logger,
                )
        ...
        if self.should_load_extra:
            ...
            if remote_path is not None:
                sync_file_gs(
                    os.path.join(remote_path, f"extra_state_world_size_{self.world_size}_rank_{self.rank}.pt"),
                    remote_extra_state_path,
                )
            ...
            if "rng" in extra_state_dict:
                ...
                log_with_rank(
                    ...
                    + (f" and {remote_extra_state_path}" if remote_path is not None else ""),
                    rank=self.rank,
                    logger=logger,
                )
        ...
        if self.local_rank == 0 and del_local_after_load:
            try:
    ...
    def save_checkpoint(
        self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None, **kwargs
    ):
        """
        ...
        # remove previous local_path, only local rank 0 should do this
        if (
            self.local_rank == 0
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if (
                    self.should_save_model or self.should_save_optimizer or self.should_save_extra
                ) and remote_path is not None:
                    sync_threads.append(
                        sync_paths_gs_threaded(local_path, remote_path, f"*_{self.world_size}_rank_{self.rank}.pt")
                    )
                    log_with_rank(f"Started checkpoint sync to {remote_path}", rank=self.rank, logger=logger)
        ...
        if self.rank == 0:
            ...
            if self.processing_class is not None:
                ...
                if remote_path is not None:
                    sync_threads.append(
                        sync_paths_gs_threaded(hf_config_tokenizer_path, os.path.join(remote_path, "huggingface"), "*")
                    )
            log_with_rank(
                f"Saved model config and tokenizer class to {os.path.abspath(hf_config_tokenizer_path)}"
                + (f" and {remote_path}" if remote_path is not None else ""),
                rank=self.rank,
                logger=logger,
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                if remote_path is not None:
                    hf_remote_path = os.path.join(remote_path, "huggingface")
                    sync_threads.append(sync_paths_gs_threaded(hf_local_path, hf_remote_path, "*"))
                log_with_rank(
                    f"Saved hf_model to {os.path.abspath(hf_local_path)}"
                    + (f" and {hf_remote_path}" if remote_path is not None else ""),
                    rank=self.rank,
                    logger=logger,
                    log_only_rank_0=True,
                )
        ...
        for sync_thread in sync_threads:
            sync_thread.join()
        if sync_threads:
            log_with_rank("Checkpoint sync completed", rank=self.rank, logger=logger)
