...
def validate_config(
    ...
    if config.data.get("val_batch_size", None) is not None:
        print(
            "NOTICE: val_batch_size is deprecated and unused."
    ...
    # filter-groups
    if config.algorithm.filter_groups.enable and config.reward_model.launch_reward_fn_async:
        raise ValueError("When filter_groups is enabled, rewards can't really be computed asynchronously")

    # Disallow AI21 exclusion when filter_groups is disabled
    if not config.algorithm.filter_groups.enable:
        ai21_exclusion_enabled = (
            config.reward_model.reward_manager == "ai21" and config.algorithm.filter_groups.filter_ai21_eval_errors
        )
        assert not ai21_exclusion_enabled, (
            "When filter_groups.enable=False, filter_ai21_eval_errors must be False for AI21 reward manager"
        )
