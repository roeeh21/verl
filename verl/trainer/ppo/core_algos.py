...
class AdvantageEstimator(str, Enum):
    ...
    GRPO_UNLIKELY = "grpo_unlikely"
    PASS_AT_K = "pass_at_k"  # AI21 implementation of Pass@k
...
@register_adv_est(AdvantageEstimator.LENGTH_REWARD)
def compute_length_reward(batch, scale_factor=1, min_length_for_reward=0):
    """Compute per-sample length reward to encourage adequate response length.

    This implements a simple linear shaping term following the Kimi 1.5 paper,
    where for each prompt group (identified by `uid`) the response with longer
    length is encouraged while preventing over-long outputs.

    Args:
        batch: DataProto-like object that contains `non_tensor_batch` with keys
            `uid` and `unpadded_responses`, and `batch` with `token_level_rewards`.
        scale_factor: Float scaling factor applied to the computed length reward.
        min_length_for_reward: Minimum response length to start applying reward.

    Returns:
        A 1-D tensor of shape (batch_size,) with length rewards to be added to
        the last token reward per sample.
    """
    # length penalty following kimi 1.5 https://arxiv.org/pdf/2501.12599
    ...
    with torch.no_grad():
        bsz = len(uids)
        for i in range(bsz):
            # make sure all responses shorter than min_length_for_reward get the same reward
            response_length = max(len(batch.non_tensor_batch["unpadded_responses"][i]), min_length_for_reward)
            uid2length[uids[i]].append(response_length)
        for uid in uid2length:
            uid2max_length[uid] = max(uid2length[uid])
            uid2min_length[uid] = min(uid2length[uid])
        for i in range(bsz):
            uid = uids[i]
            response_length = max(len(batch.non_tensor_batch["unpadded_responses"][i]), min_length_for_reward)
            if uid2max_length[uid] == uid2min_length[uid]:
                len_reward = 0
            else:
                reward = batch.batch["token_level_rewards"][i, -1]
                # map lengths between min_length to max_length to 0.5 to -0.5 (reversely linear)
                lambda_factor = 0.5 - (response_length - uid2min_length[uid]) / (
                    uid2max_length[uid] - uid2min_length[uid]
                )
                assert abs(lambda_factor) <= 0.50001, (
                    f"lambda_factor is {lambda_factor} for response length {response_length}"
                )
                len_reward = lambda_factor if reward == 1 else min(0, lambda_factor)
                len_reward *= scale_factor
            length_rewards.append(len_reward)

    # Return length rewards as a tensor for applying to token_level_rewards
    length_reward_tensor = torch.tensor(
        length_rewards, device=batch.batch["token_level_rewards"].device, dtype=batch.batch["token_level_rewards"].dtype
    )
...
def compute_policy_loss(
    ...
    if rollout_log_prob is not None:
        importance_sampling_correction = torch.exp(log_prob - rollout_log_prob)
        pg_losses = pg_losses * importance_sampling_correction
...
def _calc_pass_at_k_group_adv(scores, k):
    # based on Pass@k paper implementation https://github.com/RUCAIBox/Passk_Training/blob/main/code/passk_adv.py

    c = len(np.where(scores == 1)[0])
    n = len(scores)
    rho = 1 - comb(n - c, k) / comb(n, k)
    sigma = np.sqrt(rho * (1 - rho))
    adv_p = (1 - rho) / (sigma + 1e-6)
    adv_n = (1 - rho - comb(n - c - 1, k - 1) / comb(n - 1, k - 1)) / (sigma + 1e-6)
    new_adv = np.where(scores == 1, adv_p, scores)
    new_adv = np.where(new_adv == 0, adv_n, new_adv)
    return new_adv


@register_adv_est(AdvantageEstimator.PASS_AT_K)
def compute_pass_at_k_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    K: int,
    ...
    """
    Compute advantage for Pass@k.
    Implamented based on Pass@k Training paper https://arxiv.org/pdf/2508.10751
    by AI21 team.
    """

    scores = token_level_rewards.sum(dim=-1)
    is_success = scores >= success_threshold

    id2is_success = defaultdict(list)
    uid2sid = defaultdict(list)

    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            id2is_success[index[i]].append(is_success[i].detach().item())
            uid2sid[index[i]].append(i)
        for uid in id2is_success.keys():
            is_success_group = np.array(id2is_success[uid])
            adv = _calc_pass_at_k_group_adv(is_success_group, K)
...
@register_adv_est(AdvantageEstimator.GRPO_UNLIKELY)
def compute_grpo_unlikely_outcome_advantage(
    token_level_rewards: torch.Tensor,
    ...
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    log_probs_agg_method: str = "mean",
    success_threshold: float = 0.9,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for GRPO with unlikeliness reward penalty.
    This applies a multiplicative penalty to higher-probability solutions to encourage rarer positive samples.

    Based on "Rewarding the Unlikely: Lifting GRPO Beyond Distribution Sharpening" https://arxiv.org/abs/2506.02355
    Formula: r_i = R(x, y_i) * (1 - β_rank * (G - rank(y_i))/G)

    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, response_length)
        old_log_probs: `(torch.Tensor)`
            shape: (bs, response_length) - log probabilities from policy
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        index: `(np.ndarray)`
            UIDs for grouping responses by prompt
        beta_rank: `(float)`
            coefficient controlling strength of unlikeliness penalty (default 0.25)
        epsilon: `(float)`
            small value to avoid division by zero
        norm_adv_by_std_in_grpo: `(bool)`
            whether to scale the GRPO advantage by std
        log_probs_agg_method: `(str)`
            method to aggregate log probabilities: "mean" or "sum" or "sqrt_norm" (default "mean")
    ...
    # Compute sequence-level log probabilities (sum over response tokens)
    if log_probs_agg_method == "mean":
        seq_log_probs = (old_log_probs * response_mask).sum(dim=-1) / response_mask.sum(dim=-1)
    elif log_probs_agg_method == "sum":
        seq_log_probs = (old_log_probs * response_mask).sum(dim=-1)
    elif log_probs_agg_method == "sqrt_norm":
        seq_log_probs = (old_log_probs * response_mask).sum(dim=-1) / torch.sqrt(response_mask.sum(dim=-1))
    else:
        raise ValueError(f"Invalid log_probs_agg_method: {log_probs_agg_method}")

    scores = token_level_rewards.sum(dim=-1)
    is_success = scores >= success_threshold

    # Group samples by prompt UID and apply unlikeliness penalty
    id2indices = defaultdict(list)
    bsz = token_level_rewards.shape[0]

    # Group indices by prompt UID
    for i in range(bsz):
        id2indices[index[i]].append(i)

    with torch.no_grad():
        for prompt_uid, indices in id2indices.items():
            if len(indices) <= 1:
                # No penalty needed for single sample
                continue

            # Get log probabilities for this group
            group_log_probs = seq_log_probs[indices]
            group_is_success = is_success[indices]
            group_log_probs[~group_is_success] += 1e6  # Set large logprob to have unsuccessful samples ranked lower

            # Rank samples: higher log prob = lower rank (rank 0 = highest probability)
            # We use negative log prob for ranking so that higher prob gets lower rank
            ranks = torch.argsort(torch.argsort(-group_log_probs))

            G = len(indices)  # Number of samples in group

            # Apply unlikeliness penalty: r_i = R(x, y_i) * (1 - β_rank * (G - rank(y_i))/G)
            for i, sample_idx in enumerate(indices):
                if group_is_success[i]:
                    rank_i = ranks[i].item() + 1
                    penalty_factor = 1 - beta_rank * (G - rank_i) / G

                    # Apply penalty to all tokens in the response
                    token_level_rewards[sample_idx] = token_level_rewards[sample_idx] * penalty_factor

    # Now compute GRPO advantages using the modified rewards
