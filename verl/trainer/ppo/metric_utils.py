...
def _get_bucket_counts(scores: list[float], n_buckets: int) -> dict[int, int]:
    """count how many scores are in each bucket
    n_buckets 4 meaning 4 buckets: [0, 0.25), [0.25, 0.5), [0.5, 0.75), [0.75, 1]
    Example:
     _get_bucket_counts([0, 0.01, 0.25, 1, 0.99], 4) -> [2, 1, 0, 2]
    """
    n_prompts_in_bucket = [0 for _ in range(n_buckets)]
    for score in scores:
        # assuming max_score is between 0 and 1
        bucket_idx = max(min(int(score * n_buckets), n_buckets - 1), 0)
        n_prompts_in_bucket[bucket_idx] += 1
    return n_prompts_in_bucket


def _compute_constant_score_metrics(batch: DataProto, sequence_score: np.ndarray, n_buckets: int = 4) -> dict[str, Any]:
    """
    Compute metrics about the nature of "constant score" prompts:
    prompts that all rollouts received the same score.
    Log how many prompts received a constant score, per score, as a fraction of number of prompts.
    """
    prompt2scores = defaultdict(list)
    for uid, score in zip(batch.non_tensor_batch["uid"], sequence_score, strict=False):
        prompt2scores[uid].append(score)
    n_prompts = len(prompt2scores)
    prompt2constant_score = {
        uid: scores[0] for uid, scores in prompt2scores.items() if len(set(scores)) == 1 and scores[0]
    }

    n_prompts_in_bucket = _get_bucket_counts(prompt2constant_score.values(), n_buckets)
    metrics = {}
    for bucket_idx, n_prompts_with_score in enumerate(n_prompts_in_bucket):
        s_range = f"{bucket_idx / n_buckets:.2f}-{(bucket_idx + 1) / n_buckets:.2f}"
        metrics[f"critic/score/frac_prompts_const_score_in_{s_range}"] = n_prompts_with_score / n_prompts
    return metrics


def compute_data_metrics(
    batch: DataProto, use_critic: bool = True, is_prefilter: bool = False, prefix: str = ""
) -> dict[str, Any]:
    """
    ...
    if is_prefilter:
        metrics.update(_compute_constant_score_metrics(batch, sequence_score.detach().cpu().numpy()))
    ...
    if batch.batch.get("token_level_rewards", None) is not None:
        sequence_reward = batch.batch["token_level_rewards"].sum(-1)
        metrics.update(
            {
    ...
    if batch.batch.get("advantages", None) is not None:
        advantages = batch.batch["advantages"]
        valid_adv = torch.masked_select(advantages, response_mask)
        metrics.update(
            {
                ...
                "critic/advantages/max": torch.max(valid_adv).detach().item(),
                "critic/advantages/min": torch.min(valid_adv).detach().item(),
                "critic/advantages/all_zero_frac": torch.all(advantages * response_mask == 0, dim=1)
                .float()
                .mean()
                .detach()
                .item(),
    ...
    if batch.batch.get("returns", None) is not None:
        returns = batch.batch["returns"]
        valid_returns = torch.masked_select(returns, response_mask)
        metrics.update(
            {
    ...
    if is_prefilter:
        metrics = {f"{key}/prefilter": value for key, value in metrics.items()}
    if prefix:
