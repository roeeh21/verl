...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self._optimizer_offload_per_mini_step = False
        if self._is_actor:
            self._is_offload_param = self.config.actor.fsdp_config.get("param_offload", False)
            self._is_offload_optimizer = self.config.actor.fsdp_config.get("optimizer_offload", False)
            self._optimizer_offload_per_mini_step = self.config.actor.fsdp_config.get(
                "optimizer_offload_per_mini_step", False
            )
        elif self._is_ref:
        ...
        self._rollout_model_active = None
    ...
    def _build_model_optimizer(
        ...
        if fsdp_strategy == "fsdp2":
            # fsdp2: use get_init_weight_context_manager to match upstream verl
            ...
            # init_context = get_init_weight_context_manager(
            #     use_meta_tensor=not actor_model_config.tie_word_embeddings, mesh=self.device_mesh
            # ) # TODO: TAMERG - check later if the comment above relevant
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if fsdp_strategy == "fsdp2":
                # For fsdp2, load weights directly (matching upstream verl apporach)
                # NOTE: Jamba Large isn’t supported with the current fsdp2 setup due to CPU memory limits.
                ...
                # For fsdp1, load model with config:
        ...
        if mixed_precision_config is not None:
            ...
            # ** buffer_dtype defaults to fp32 TODO: TAMERG should switch to fp32?
        ...
        if fsdp_strategy == "fsdp":
            print("before parallel_load_safetensors")
            shard_states = parallel_load_safetensors(local_path)
            parallel_init_fn = parallel_init_module_fn(actor_module, shard_states)
            actor_module_fsdp = FSDP(
                actor_module,
                cpu_offload=cpu_offload,
                param_init_fn=parallel_init_fn,
                auto_wrap_policy=auto_wrap_policy,
                device_id=get_device_id(),
                sharding_strategy=sharding_strategy,  # zero3
                mixed_precision=mixed_precision,
                # sync_module_states=True, ** in upstream its passed.
                device_mesh=self.device_mesh,
                use_orig_params=self.use_orig_params,
                forward_prefetch=fsdp_config.get("forward_prefetch", False),
            )
        elif fsdp_strategy == "fsdp2":
            # fsdp2 doesnt support parallel_init_fn hook (matching upstream verl apporach)
            # here weights are already loaded from_pretrained
        ...
        if role == "actor" and optim_config is not None:
            ...
            for n, p in actor_module_fsdp.named_parameters():
                if any([fnmatch.fnmatch(n, pat) for pat in (optim_config.freeze_params or [])]):
                    ignore_param_names.append(n)
                else:
                    train_params.append(p)
            if optim_config.freeze_params:
                assert ignore_param_names, (
                    f"freeze_params set to '{optim_config.freeze_params}' but no param names matcheed"
                )
                print(f"Freezing/Not training params: {ignore_param_names}")

            actor_optimizer = build_optimizer(train_params, optim_config)
    ...
    async def rollout_mode(self, timing: dict = None):
        """Context switch hybridengine to rollout mode."""
        if self._rollout_model_active == True:  # noqa: E712
            return
        self._rollout_model_active = True

        timing = timing if timing is not None else {}
        with simple_timer("activate_rollout_mode", timing):
            self.empty_cache()
            ...
            log_gpu_memory_usage("After state_dict() in rollout_mode()", logger=logger)
            ...
            if self.config.rollout.offload_state_dict_before_load:
                params = {k: v.cpu() for k, v in params.items()}
                torch.cuda.empty_cache()
                log_gpu_memory_usage("After offload state_dict to CPU in sharding manager memory", logger=logger)
            ...
            if peft_config is not None and self.base_sync_done:
                ...
                if fsdp_version(self.actor_module_fsdp) == 1:
                    world_size = torch.distributed.get_world_size()
                    assert hasattr(torch, self.rollout.config.dtype)
                    cast_params_to = (
                        getattr(torch, self.rollout.config.dtype)
                        if self.config.actor.fsdp_config.cast_params_to_inference_dtype_before_update
                        else None
                    )

                    def get_full_tensor(param):
                        if self.config.rollout.offload_state_dict_before_load:
                            param = param.cuda()  # TODO: TAMERG does it have to be blocking call opposed to upstream?
                        if world_size != 1 and hasattr(param, "full_tensor"):
                            param = param.full_tensor()
                        if cast_params_to is not None:
                            param = param.to(cast_params_to)
                        return param

                    per_tensor_param = ((name, get_full_tensor(param)) for name, param in params.items())
                else:
            ...
            self.empty_cache()
            log_gpu_memory_usage("After del state_dict and empty_cache in rollout_mode()", logger=logger)
    ...
    async def trainer_mode(self, timing: dict = None):
        """Context switch hybridengine to trainer mode."""
        if self._rollout_model_active == False:  # noqa: E712
            return
        self._rollout_model_active = False

        timing = timing if timing is not None else {}
        with simple_timer("activate_training_mode", timing):
            if self.config.rollout.free_cache_engine:
    ...
    def init_model(self, can_delete_source_model=False):
        ...
        move_left_padding_right = self.config.model.get("move_left_padding_right", False)
        minimize_padding = self.config.model.get("minimize_padding", False)
        ...
        if self._is_ref:
            ...
            with open_dict(self.config.ref):
                ...
                self.config.ref.move_left_padding_right = move_left_padding_right
                self.config.ref.minimize_padding = minimize_padding
        ...
        if self.config.model.delete_source_model_after_init and can_delete_source_model:
            # os.environ["LOCAL_RANK"] should be more correct here but for some reason its 0 for all ranks
            local_rank = torch.distributed.get_rank() % 8
            torch.distributed.barrier()  # Make sure everyone finished their initialization before deleting
            if local_rank == 0:
                print(f"Deleting source model: {self.config.model.path}")
                shutil.rmtree(self.config.model.path)
            torch.distributed.barrier()
    ...
    def update_actor(self, data: DataProto):
        ...
        log_gpu_memory_usage("Before onload actor model during update_actor", logger=logger)
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        log_gpu_memory_usage("After onload actor model during update_actor", logger=logger)

        def onload_optimizer():
            load_fsdp_optimizer(optimizer=self.actor_optimizer, device_id=get_device_id())
            log_gpu_memory_usage("After onload actor optimizer during update_actor", logger=logger)

        def offload_optimizer():
            log_gpu_memory_usage("Before offload actor optimizer during update_actor", logger=logger)
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor", logger=logger)

        if self._is_offload_optimizer and not self._optimizer_offload_per_mini_step:
            onload_optimizer()
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                metrics = self.actor.update_policy(
                    data=data,
                    optim_onload_fn=onload_optimizer
                    if self._is_offload_optimizer and self._optimizer_offload_per_mini_step
                    else None,
                    optim_offload_fn=offload_optimizer
                    if self._is_offload_optimizer and self._optimizer_offload_per_mini_step
                    else None,
                )
            ...
            for k, v in metrics.items():
                if isinstance(v, list):
                    non_tensor_batch[f"metric/{k}"] = np.array(v)
                elif isinstance(v, int) or isinstance(v, float):
                    non_tensor_batch[f"metric/{k}"] = np.array([v])
                else:
                    raise ValueError(f"Unexpected metric type {type(v)} for key {k}")
            output = DataProto(non_tensor_batch=non_tensor_batch)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    @DistProfiler.annotate(color="red", role="activate_trainer_mode")
    def activate_trainer_mode(self):
        loop = get_event_loop()
        loop.run_until_complete(self.trainer_mode())

    def _generate_sequences(self, prompts: DataProto, keep_rollout_mode_active: bool = False):
        ...
        if self._is_actor:  # For rollout only, we do not switch context.
            loop = get_event_loop()
            loop.run_until_complete(self.rollout_mode(timing_generate))
        ...
        if self._is_actor:
            if not keep_rollout_mode_active:
                loop.run_until_complete(self.trainer_mode(timing_generate))
    ...
    def generate_sequences(self, prompts: DataProto):
        return self._generate_sequences(prompts=prompts, keep_rollout_mode_active=False)

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="red", role="rollout_generate_keep_rollout_mode_active")
    def generate_sequences_keep_rollout_mode_active(self, prompts: DataProto):
        return self._generate_sequences(prompts=prompts, keep_rollout_mode_active=True)
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        torch.cuda.empty_cache()  # TODO: maybe we forgot this here from jamba-large work and can remove

        return output

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None, remote_path=None):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False, remote_path=None):
    ...
    def empty_cache(self):
        if self.config.actor.fsdp_config.aggressive_empty_cache:
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        if mixed_precision_config is not None:
            param_dtype = PrecisionType.to_dtype(mixed_precision_config.get("param_dtype", "bf16"))
            reduce_dtype = PrecisionType.to_dtype(mixed_precision_config.get("reduce_dtype", "fp32"))
            # ** buffer_dtype defaults to fp32 TODO: TAMERG change back to fp32
            buffer_dtype = PrecisionType.to_dtype(mixed_precision_config.get("buffer_dtype", "bf16"))
        else:
