...
@dataclass
class FilterDisagreementLogprobsConfig(BaseConfig):
    enable: bool = False
    threshold: float = 1.0
...
class ActorConfig(BaseConfig):
    ...
    router_aux_loss: bool = False
    router_lbloss_coef: float = 0.15
    router_zloss_alpha: float = 1.0e-05
    logits_zloss_alpha: float = 0.0
    filter_disagreement_logprobs: FilterDisagreementLogprobsConfig = field(
        default_factory=FilterDisagreementLogprobsConfig
    )
    maskout_clipped_responses: bool = False
...
class FSDPActorConfig(ActorConfig):
    ...
    Args:
        ...
        move_left_padding_right (bool): Whether to move left padding to the right.
        minimize_padding (bool): Whether to minimize padding in inputs during training.
    """
