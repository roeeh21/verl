...
class vLLMHttpServerBase:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        if self.config.mamba_ssm_cache_dtype_fp32:
            engine_kwargs["mamba_ssm_cache_dtype"] = "float32"
        ...
        args = {
            ...
            "no_enable_prefix_caching": not self.config.enable_prefix_caching,
    ...
    async def generate(
        ...
        remaining_context_tokens = max(0, self.config.max_model_len - len(prompt_ids))
        max_tokens = min(int(requested_max_tokens), remaining_context_tokens)
