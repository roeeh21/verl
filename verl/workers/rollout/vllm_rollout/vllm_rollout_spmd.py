...
def generate_chunked(lm, prompts, sampling_params, stop_token_ids, chunk_size=None, lora_request=None):
    assert not sampling_params.stop, "TODO: support stop sequences in chunked generation"

    max_tokens = org_max_tokens = sampling_params.max_tokens
    n = sampling_params.n
    prompts = [dict(prompt) for prompt in prompts for i in range(n)]  # Make a copy since we modify inpalce
    sampling_params.n = 1
    chunk_size = chunk_size or max_tokens
    results = [None] * len(prompts)
    dones = [False] * len(prompts)
    all_done = False
    while not all_done and max_tokens > 0:
        cur_reqs = []
        cur_indices = []
        all_done = True
        for i, prompt in enumerate(prompts):
            if dones[i]:
                continue
            cur_reqs.append(prompt)
            cur_indices.append(i)
        if not cur_reqs:
            break
        sampling_params.max_tokens = min(chunk_size, max_tokens)
        all_res = lm.generate(
            prompts=cur_reqs,
        ...
        assert len(all_res) == len(cur_indices)
        for i, res in enumerate(all_res):
            ...
            if results[idx] is None:
                ...
                results[idx].outputs[0].text += res.outputs[0].text
                results[idx].outputs[0].token_ids += res.outputs[0].token_ids
                results[idx].outputs[0].logprobs += res.outputs[0].logprobs
                results[idx].outputs[0].cumulative_logprob += res.outputs[0].cumulative_logprob
                results[idx].outputs[0].finish_reason = res.outputs[0].finish_reason
            prompts[idx]["prompt_token_ids"] = prompts[idx]["prompt_token_ids"] + list(res.outputs[0].token_ids)
            # print(res.outputs[0].finish_reason)

            # TODO: For stop-sequence support we need to check for stop sequences here,
            # but they aren't in the output text by default, maybe need to check the tokens
            if res.outputs[0].finish_reason != "length" or any(
                [res.outputs[0].token_ids[-1] == tok for tok in stop_token_ids]
            ):
                dones[idx] = True
            else:
                all_done = False
        max_tokens -= chunk_size
    sampling_params.max_tokens = org_max_tokens
    sampling_params.n = n
    return results


class LogitBiasProcessor:
    def __init__(self, logit_bias_dict):
        self._bias = None
        self._logit_bias_dict = logit_bias_dict

    def __call__(self, token_ids, logits):
        if self._bias is None:
            self._bias = torch.zeros([logits.shape[-1]], dtype=logits.dtype, device=logits.device)
            for k, v in self._logit_bias_dict.items():
                self._bias[k] = v
        bias = self._bias
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # michaelg: commented-out this check. let vLLM assert the validity of max_num_batched_tokens
        ...
        self.inference_engine = LLM(
            model=model_path,
            enable_sleep_mode=True,  # in the upstream tied to config.free_cache_engine
            ...
            mamba_ssm_cache_dtype="float32" if config.mamba_ssm_cache_dtype_fp32 else "auto",
            **compilation_config,
            **self.lora_kwargs,
            **engine_kwargs,
        )
        ...
        if config.disable_output_tokens:
            logit_bias = {
                idx: -50000.0
                for idx in range(tokenizer.vocab_size)
                if any(
                    [fnmatch.fnmatch(tokenizer.convert_ids_to_tokens(idx), pat) for pat in config.disable_output_tokens]
                )
            }
            print(f"disabling {len(logit_bias)} tokens with logit bias")
            kwargs["logits_processors"] = [LogitBiasProcessor(logit_bias)]
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            if not self.config.generate_chunk_size:
                ...
                outputs = generate_chunked(
                    self.inference_engine,
                    prompts=vllm_inputs,  # because we have already convert it to prompt token id
                    sampling_params=self.sampling_params,
                    chunk_size=self.config.generate_chunk_size,
                    stop_token_ids=[self.pad_token_id],
                    lora_request=lora_requests,
                )
            ...
            non_tensor_batch["unpadded_responses"] = np.fromiter(response, dtype=object)
            responses_lengths = torch.tensor([len(r) for r in response], device=idx.device)
        ...
        response_attention_mask = get_response_mask(
            ...
            responses_lengths=responses_lengths,
