...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        offload_state_dict_before_load: bool = False,
        cast_params_to_inference_dtype_before_update: bool = False,
    ):
        self.module = module
        ...
        self.vllm_dtype = inference_engine.llm_engine.get_model_config().dtype
        ...
        self.offload_state_dict_before_load = offload_state_dict_before_load
        self.cast_params_to_inference_dtype_before_update = cast_params_to_inference_dtype_before_update
    ...
    def __enter__(self):
        print("entering vllm sharding manager")
        ...
        with simple_timer("reshard", self.timing):
            ...
            if self.offload_param:
                ...
                log_gpu_memory_usage("After load_fsdp_model_to_gpu in sharding manager memory", logger=logger)
            ...
            if self.offload_param:
                # TODO: It might not be correct to offload here for cases
                # where params is just a view of the model params.
                # But that's not the case by default.
                offload_fsdp_model_to_cpu(self.module)
                log_gpu_memory_usage("After offload_fsdp_model_to_cpu in sharding manager memory", logger=logger)

            if self.offload_state_dict_before_load:
            ...
            # update model params
            cast_params_to = self.vllm_dtype if self.cast_params_to_inference_dtype_before_update else None
            self.update_params(params, peft_config=peft_config, cast_params_to=cast_params_to)
            log_gpu_memory_usage("After sync model weights in sharding manager", logger=logger)
            del params

            get_torch_device().empty_cache()

            if self.rollout_config.free_cache_engine and (
                "tags" in inspect.signature(self.inference_engine.wake_up).parameters
            ):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        print("exiting vllm sharding manager")
        if self.rollout_config.free_cache_engine:
    ...
    def update_params(self, updated_params, peft_config=None, cast_params_to=None):
        ...
        Args:
            ...
            cast_params_to (optional): Dtype to cast parameters to.
        """
        ...
        if fsdp_version(self.module) == 1:
            ...
            loaded_params = model.load_weights(
                ((name, get_full_tensor(param)) for name, param in updated_params.items())
            )
