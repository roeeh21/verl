...
def mixtral_dtensor_weight_loader(actor_weights: dict, vllm_model: nn.Module) -> nn.Module:
    stacked_params_mapping = [
        # (param_name, shard_name, shard_id)
        ("qkv_proj", "q_proj", "q"),
        ("qkv_proj", "k_proj", "k"),
    ...
    expert_params_mapping = FusedMoE.make_expert_params_mapping(
        ...
        num_experts=vllm_model.config.num_local_experts,
    )
    ...
    for name, loaded_weight in actor_weights.items():
        ...
        for param_name, weight_name, shard_id in stacked_params_mapping:
            ...
            for mapping in expert_params_mapping:
                ...
                weight_loader(
                    param,
                    local_loaded_weight.to(dtype=target_dtype),
                    name,
                    shard_id=shard_id,
                    expert_id=expert_id,
                )
...
def jamba_dtensor_weight_loader(actor_weights: dict, vllm_model: nn.Module) -> nn.Module:
    stacked_params_mapping = [
        # (param_name, shard_name, shard_id)
    ...
    name_remaps = {
        ...
        "feed_forward.down_proj": "feed_forward.experts.0.down_proj",
        "feed_forward.up_proj": "feed_forward.experts.0.up_proj",
        "feed_forward.gate_proj": "feed_forward.experts.0.gate_proj",
    }
    expert_params_mapping = FusedMoE.make_expert_params_mapping(
        ckpt_gate_proj_name="gate_proj",
        ckpt_down_proj_name="down_proj",
        ckpt_up_proj_name="up_proj",
        num_experts=vllm_model.config.num_experts,
    )
    ...
    for name, loaded_weight in actor_weights.items():
        ...
        for param_name, weight_name, shard_id in stacked_params_mapping:
            ...
            for mapping in expert_params_mapping:
                ...
                local_loaded_weight = redistribute_dtensor(param_name=name, loaded_weights=loaded_weight)
                weight_loader = getattr(param, "weight_loader", default_weight_loader)
                target_dtype = param.dtype if param.dtype != torch.int8 else torch.bfloat16
                weight_loader(
                    param,
                    local_loaded_weight.to(dtype=target_dtype),
                    name,
...
__MODEL_DTENSOR_WEIGHT_LOADER_REGISTRY__ = {
    ...
    "JambaForCausalLM": jamba_dtensor_weight_loader,
    "MixtralForCausalLM": mixtral_dtensor_weight_loader,
}
...
def _get_model_weight_loader(arch: str):
    ...
    raise ValueError(
        ...
        f"{__MODEL_DTENSOR_WEIGHT_LOADER_REGISTRY__.keys()}"
    )
